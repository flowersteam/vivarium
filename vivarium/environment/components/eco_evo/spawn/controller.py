...
from .....controllers.controller import Controller, AttributeMapping, set_nested_attr
...
class SpawnController(Controller):
    def __init__(self, name, remote, mapping=None):
        ...
        mapping = mapping or {
            'position_range': AttributeMapping(
                f'state.{name}_state.position_range',
                remote_to_ctrl_fn=lambda x: tuple(x.tolist()),
                ctrl_to_remote_fn=lambda x: np.array(x)
            ),
            'orientation_range': AttributeMapping(
                f'state.{name}_state.orientation_range',
                remote_to_ctrl_fn=lambda x: tuple(x.tolist()),
                ctrl_to_remote_fn=lambda x: np.array(x)
            ),
            'subtype': AttributeMapping(
                f'state.{name}_state.subtype',
