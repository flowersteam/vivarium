...
tmp_state = scene_config.create_state()
agent_field = tmp_state.field_name(AgentState)
object_field = tmp_state.field_name(ObjectState)
...
def test_entity_wrapper(idx, position_center, position_orientation, init_state_fn, entity_type):
    state = init_state_fn()
...
def test_entity_list(idx, position_center, entity_type, state_fn):
    state = state_fn()
...
def test_on_simulator_instance():
...
def test_simulator_apply_change():
    simulator = scene_config.create_simulator()
    dw = DataclassWrapper()
    dw.box_size = 42.
    dw.freq = -10.
